...
六阶段管道：
  ...
  Stage 6  口语词清理      — 保留，仅供研究，【禁止在 purple loop 中启用】

⚠️  核心约束（绝对不能违反）：
    purple loop 的文稿来自字幕识别，每个字对应 PR 里的时间码。
    任何修改文字内容的操作（删词、改词）都会导致 PR 文字搜索对不上。
    Stage 6（口语词清理）永远不得在生产流程中调用，clean_fillers 永远为 False。
